...
public:
    ...
    virtual double Evaluate(const AccessToSheet& sheet_access_func) const = 0;
...
public:
    ...
    double Evaluate(const AccessToSheet& sheet_access_func) const override {
        auto left = lhs_->Evaluate(sheet_access_func);
        auto right = rhs_->Evaluate(sheet_access_func);
        ...
        switch (type_) {
            ...
            case Multiply:
                ...
                break;
            case Divide:
                double div_result = left / right;
                if (!std::isfinite(div_result)) {
...
public:
    ...
    double Evaluate(const AccessToSheet& sheet_access_func) const override {
        if (type_ == UnaryMinus) {
            return -operand_->Evaluate(sheet_access_func);
        } else {
            assert(type_ == UnaryPlus);
            return +operand_->Evaluate(sheet_access_func);
        }
    }
...
public:
    ...
    void DoPrintFormula(std::ostream& out, ExprPrecedence /* precedence */) const override { Print(out); }

    ExprPrecedence GetPrecedence() const override { return EP_ATOM; }

    double Evaluate(const AccessToSheet& sheet_access_func) const override { return sheet_access_func(*cell_); }
...
public:
    ...
    double Evaluate(const AccessToSheet&) const override { return value_; }
...
double FormulaAST::Execute(const AccessToSheet& sheet_access_func) const {
    return root_expr_->Evaluate(sheet_access_func);
}
